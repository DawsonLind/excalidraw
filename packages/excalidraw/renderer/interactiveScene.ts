  ...
  switch (suggestedBinding.element.type) {
    ...
    default:
      ...
      switch (suggestedBinding.element.type) {
        ...
        case "triangle":
          {
            const [segments] = deconstructTriangleElement(
              suggestedBinding.element,
            );
  ...
  if (
    ...
    if (!cursorIsInsideBindable || isElbow) {
      ...
      if (suggestedBinding.element.type === "diamond") {
        ...
        const basePoints = [
          {
            x: suggestedBinding.element.width / 4,
          ...
          {
            x: (suggestedBinding.element.width * 3) / 4,
            y: suggestedBinding.element.height / 2,
          },
        ];
  ...
  switch (element.type) {
    ...
    default:
      ...
      switch (element.type) {
        ...
        case "triangle":
          {
            const [segments] = deconstructTriangleElement(element, offset);
    ...
    if (appState.isMidpointSnappingEnabled) {
      ...
      if (element.type === "diamond") {
        ...
        const basePoints = [
          ...
          { x: (element.width * 3) / 4, y: element.height / 2 },
        ];
        midpoints = basePoints.map((point) => {
          const globalPoint = pointFrom<GlobalPoint>(
            point.x + element.x,
